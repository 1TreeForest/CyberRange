...
def slave(task_queue, domain_list, item_dict, domain_list_len):
    ...
    for task in iter(task_queue.get, 'STOP'):
        ...
        if exist_count == domain_list_len - task[0] - 1:  # 对库中初始样本进行断点续连式对比
        ...
        for domain_2 in domain_list[task[0] + 1:]:
            ...
            similarity = Levenshtein.ratio(item_dict[domain_1][:task[1]], item_dict[domain_2][:task[1]])
...
def master():
    ...
    for i in range(domain_list_len):
        for tag_len in [250, 500]:
            task_queue.put([i, tag_len])

    #  give tasks
    for i in range(NUMBER_OF_PROCESSES):
        Process(target=slave, args=(task_queue, domain_list, item_dict, domain_list_len)).start()
